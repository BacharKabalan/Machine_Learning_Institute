...
def evaluation(transformer_model,val_dl,loss_function, device):
    ...
    with torch.no_grad():
        for val_idx, val_batch in enumerate(val_dl):
            enc_tokens = val_batch['contx'].to(device)
            dec_tokens = val_batch['input'].to(device)
            val_labels = val_batch['label'].to(device)
            val_output= transformer_model(enc_tokens, dec_tokens)
    ...
    bleu_results = bleu_metrics.bleu_evaluate(val_model_output, val_true_labels,enc_tokens.size(0))
