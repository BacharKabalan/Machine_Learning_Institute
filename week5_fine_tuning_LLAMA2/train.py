...
def compute_metrics(p):
    ...
    softmax_tensor = torch.nn.functional.softmax(predictions, dim=2)
    
    # Find the index of the maximum value along the last dimension
    max_indices = torch.argmax(softmax_tensor, dim=2)
    # Decode the model outputs to get the generated text
    generated_text = ds.tokenizer.decode(max_indices[0], skip_special_tokens=True)
